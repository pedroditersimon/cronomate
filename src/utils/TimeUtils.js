export function toDate(timeStamp, nowOnUndefined = true) {
...
export function convertElapsedTimeToText(elapsedMs) {
    if (elapsedMs <= 0)
        return undefined;
...
export function convert24HourFormatTextToTime(timeTxt, baseTime) {
    // no contiene numeros
    if (!/\d/.test(timeTxt))
        return undefined;
...
export function formatDateToText(date, locale = navigator.language || 'es-ES') {
    if (isToday(date)) {
        return "Hoy";
    }
    const isPastYear = date.getFullYear() < new Date().getFullYear();
    const options = isPastYear
...
function getElapsedTimeFromNow(time) {
    if (time === undefined)
        return 0;
